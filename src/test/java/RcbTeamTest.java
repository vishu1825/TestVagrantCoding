...
public class RcbTeamTest {
    ...
    public void getTotalPrice(Method m){
        FrameworkUtils util =new FrameworkUtils();
        double actualPrice = util.getTotalPrice(FrameworkConstant.getCOUNTRY_NAME());
        Assert.assertEquals(actualPrice,FrameworkConstant.TOTAL_PRICE,"Total price of all 4 Foreign player ");
        Reporter.log(m.getName()+" is passed " + actualPrice,true);
    }
